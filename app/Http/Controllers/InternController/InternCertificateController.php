    ...
    public function index(Request $request, $org_id)
    {
        try {
            ...
            if ($request->input('mode') == 1) {
                $certificate = InternCertificate::with('Intern', 'IssuedBy')->where('organization_id', $org_id)->get();
                ...
                $mappedCertificate = $certificate->map(function ($dep) {
                    return [
                        'intern_name' => trim(($dep->Intern->first_name ?? '') . ' ' . ($dep->Intern->last_name ?? '')),

                        'certificate_type' => $dep->certificate_type ?? '',
                        'certificate_title' => $dep->certificate_title ?? '',
                        'certificate_number' => $dep->certificate_number ?? '',
                        'issue_date' => $dep->issue_date ?? '',
                        'remarks' => $dep->remarks ?? '',
                        'IssuedBy' => trim(
                            ($dep->IssuedBy?->first_name ?? '') . ' ' .
                            ($dep->IssuedBy?->middle_name ?? '') . ' ' .
                            ($dep->IssuedBy?->last_name ?? '')
                        ),
            ...
            $query = InternCertificate::with('Intern', 'IssuedBy')->where('organization_id', $org_id);
    ...
    public function store(Request $request, $org_id)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'issued_by_employee_id' => 'required|integer|exists:employees,employee_id',
            ...
            $employeeDocument = InternCertificate::create($data);
            return response()->json([
